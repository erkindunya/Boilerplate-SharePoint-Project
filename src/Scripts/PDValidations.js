...
function ValidateValueDetails() {
	var rbIsNonDisclosureValue =  $("input[name=rbIsNonDisclosure]:radio").prop('checked');
...
function ValidateDates() {
    if ($("#dtPlannedStartDate").val()) {
            ...
            if ($("#dtPlannedCompletionDate").val()) {
                if ($("#dtActualStartDate").val()) {
					if ($("#txtPreConstructionduration").val()) {
						if (selectedSoP == postHandover || ((selectedSoP == "Award / Pre-Construction") && (rbIsNonDisclosureValue == "true"))) {
		...
		else if (selectedSoP == "Award / Pre-Construction" && rbIsNonDisclosureValue == "true") {
			if ($("#txtPreConstructionduration").val()) {
				return true;
			} 
			else {
...
if ($("#ppBidLead").val()) {
			//Validations based on the 'Stage of Project' 
			if (selectedSoP == construction || selectedSoP == postHandover || ((selectedSoP == "Award / Pre-Construction") && (rbIsNonDisclosureValue == "true"))) {
...
function validateNonDisclosure() {
	if (ValidateForDraft()) {
		if ($("#txtContractNumber").val()) {
			if ($("#txtClient").val()) {
				if ($("#ddlDeliveryRegion option:selected").val() != 'default') {
