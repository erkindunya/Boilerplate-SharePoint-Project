...
function loadMasterData() {
    ...
    projectEBID = getParameterByName('ID');
    if (projectEBID) {
        loadProjectEB(projectEBID);

    } else {
        $("input[type=submit]").prop("disabled", true);
        alert('Invalid Project Evidence Bite ID.');
    }
}
...
function loadProjectEB(listItemID) {
    listItem = null;
    var projectEBList = context.get_web().get_lists().getByTitle(listProjectEvidenceBite);
    listItem = projectEBList.getItemById(listItemID);
...
function onLoadListItemsSucceeded() {
    ...
    if (listItem.get_item('CRM_x0020_Opportunity_x0020_Auth'))
        $("#ppCRMAuthor").val(listItem.get_item('CRM_x0020_Opportunity_x0020_Auth').get_lookupValue());
    if (listItem.get_item("Stage_x0020_of_x0020_Project"))
    ...
    if (listItem.get_item('Reason_x0020_for_x0020_extension') != null && listItem.get_item('Reason_x0020_for_x0020_extension') != '') {
        $('#extensionCheck').show();
    ...
    //Non-Disclosure
    if (listItem.get_item("isNonDisclosureAgreement"))
        $("input[name=rbIsNonDisclosure][value='Yes']").attr('checked', 'checked');
    else
        $("input[name=rbIsNonDisclosure][value='No']").attr('checked', 'checked');

    //Is Best Bid
    if (listItem.get_item("isBestBid"))
        $('#chkIsBestBid').prop('checked', true);
    ...
    if (listItem.get_item('Structural_x0020__x0026__x0020_C'))
        $("#txtStructural").val(listItem.get_item('Structural_x0020__x0026__x0020_C'));
    ...
    //Pre-Construction BREEAM/DREAM rating
    if (listItem.get_item('Pre_x002d_Construction_x0020_BRE') != null)
        $("#ddlPreConstructionRating").val(listItem.get_item('Pre_x002d_Construction_x0020_BRE').get_label());

    //Pre-Construction BREEAM/DREAM rating Score
    $("#txtPreConstructionScore").val(listItem.get_item('Pre_x002d_Construction_x0020_BRE0'));
    ...
    if (listItem.get_item('Business_x0020_Function'))
        $("#ddlBusinessFunctions").val(listItem.get_item('Business_x0020_Function').get_label());

    $('#ddlBusinessFunctions').trigger('change');
    topicValue = listItem.get_item('Topic') ? listItem.get_item('Topic').get_label() : null;
    ...
    // library link in the evidence bite section
    if (listItem.get_item('Kier_x0020_album_x0020_link')) {
        $('#txtImageLibLink1').val(listItem.get_item('Kier_x0020_album_x0020_link'));
        $('#txtImageLibLink1').attr('href', listItem.get_item('Kier_x0020_album_x0020_link'));
    }

    if (listItem.get_item('Architect_x0020_Not_x0020_applic'))
        $('#chkArchitectNA').prop('checked', true);
    if (listItem.get_item('Acoustician_x0020_Not_x0020_appl'))
    ...
    if (listItem.get_item("Client_x0020_Reference_x0020_ava"))
        ...
        $("input[name=rbClientReference1][value='No']").attr('checked', 'checked');

    // Binding Project Documents
    var projectDatasheetID = listItem.get_item('ProjectDatasheetID').get_lookupValue();
...
function BindProjectDocuments(projectDSID, TestimonialType) {
    //var url= _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/getbytitle('Testimonial%20documents')/items?$select=File,ProjectDatasheetIDId&$expand=File&$filter=ProjectDatasheetID eq '"+projectDSID+"'";
    var url = _spPageContextInfo.webAbsoluteUrl + "/_api/web/getfolderbyserverrelativeurl('Testimonial%20documents/" + projectDSID + "/" + TestimonialType + "')/files"
    $.ajax({
        url: url,
