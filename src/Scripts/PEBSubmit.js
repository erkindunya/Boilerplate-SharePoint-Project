    ...
    try {
        ...
        if ('true' == isDarft) {
            if (ValidateFormDraft()) {
                updateProjectBite(true);
            } else {
                return false;
            }
        } else {
            if (ValidateForm()) {
                updateProjectBite(false);
            } else {
                return false;
            }
        }
...
function updateProjectBite(updateDraft) {
    try {
        ShowWaitDialog();
        ...
        if (listItem.get_item('CRM_x0020_Opportunity_x0020_Auth0'))
            crmAuthorID = listItem.get_item('CRM_x0020_Opportunity_x0020_Auth0').get_lookupId();

        if (crmAuthorID)
            oListItem.set_item('CRM_x0020_Opportunity_x0020_Auth', crmAuthorID);
        ...
        if ($("input:radio[name='rbIsNonDisclosure']:checked").val() == "Yes")
            oListItem.set_item('isNonDisclosureAgreement', "1");
        else
            oListItem.set_item('isNonDisclosureAgreement', "0");
        ...
        //Champion
        var champ = $('#txtAuthor').val();
        if (champ)
        	oListItem.set_item('Champion', user.get_id());
        ...
        oListItem.set_item('Structural_x0020__x0026__x0020_C', $("#txtStructural").val());
        ...
        if ($("#chkStructuralCT").is(':checked'))
            oListItem.set_item('Structural_x0020__x0026__x0020_C1', '1');
        else
            oListItem.set_item('Structural_x0020__x0026__x0020_C1', '0');
        ...
        oListItem.set_item('Pre_x002d_Construction_x0020_BRE0', $("#txtPreConstructionScore").val());
        ...
        //Library link field in the evidence bite section
        if ($("#txtImageLibLink1").val())
            oListItem.set_item('Kier_x0020_album_x0020_link', $("#txtImageLibLink1").val());

        //Inserting 'Project Datasheet ID' Look up value
        if (projectDatasheetID) {
            var fieldPDID = new SP.FieldLookupValue();
            fieldPDID.set_lookupId(projectDatasheetID);
            oListItem.set_item('ProjectDatasheetID', fieldPDID);
        }
        //Inserting 'Project Datasheet ID' Look up value
        ...
        if (updateDraft === true) {
            oListItem.set_item('WF_x0020_Status', "Draft");
        }
...
function onQuerySucceeded() {
    ...
    waitDialog = null;
    returnValue = true;
    successMessage = "Successfully submitted the 'Project Evidence Bite' and your Reference ID : " + oListItem.get_id() + ".<br />" + "Do you want to add another 'Project Evidence Bite' ?";
...
function onQueryFailed(sender, args) {
    ...
    } else if(args.get_message().indexOf('does not exist')>=0)
	{
		onQuerySucceeded();
	} 
    else {
